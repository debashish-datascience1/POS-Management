...
<div class="container">
    ...
    <form action="{{ route('final-product.update', $finalProduct->id) }}" method="POST">
        @csrf
        @method('PUT')
        <div class="form-group">
            <label for="product_name">Product Name</label>
            <input type="text" class="form-control" id="product_name" name="product_name" value="{{ old('product_name', $finalProduct->product_name) }}" required>
        </div>
        
        <div class="form-group">
            <label for="description">Description</label>
            <textarea class="form-control" id="description" name="description" rows="3">{{ old('description', $finalProduct->description) }}</textarea>
        </div>
        
        <div class="form-group">
            <label for="quantity">Quantity</label>
            <input type="number" class="form-control" id="quantity" name="quantity" value="{{ old('quantity', $finalProduct->quantity) }}" required min="0">
        </div>
        
        <div class="form-group">
            <label for="sum">Sum</label>
            <input type="number" step="0.01" class="form-control" id="sum" name="sum" value="{{ old('sum', $finalProduct->sum) }}" required min="0">
        </div>
        
        <button type="submit" class="btn btn-primary">Update Final Product</button>
...
@section('content')
    ...
    <section class="content">
        @component('components.widget', ['class' => 'box-primary'])
            {!! Form::model($packing, [
                'url' => action([\App\Http\Controllers\FinalProductController::class, 'update'], [$packing->id]),
                'method' => 'put',
                'id' => 'packing_form',
            ]) !!}
            <div class="row">
                <div class="col-sm-6">
                    <div class="form-group">
                        ...
                        {!! Form::date('date', \Carbon\Carbon::parse($packing->date), ['class' => 'form-control', 'required']) !!}
                    </div>
                </div>
                <div class="col-sm-6">
                    <div class="form-group">
            ...
            <div class="packing-section">
                @php
                    ...
                    $temperatures_data = is_string($packing->temperature)
                ...
                @foreach ($temperatures_data as $index => $temperature)
                                    ...
                                    <div class="col-sm-3">
                                        <div class="form-group">
                                            ...
                                            {!! Form::select('product_temperature[]', $product_temperatures, 
                                                isset($packing->product_temperature[$index]) ? $packing->product_temperature[$index] : null, 
                                                [
                        ...
                        <div class="row">
                            <div class="col-sm-12">
                                <div class="form-group">
                                    ...
                                    {!! Form::text('total[]', $packing->total[$index], [
                ...
                <div class="col-sm-12">
                    <div class="btn-group">
                        ...
                        <button type="button" class="btn btn-danger remove-section" 
                            {{ count($temperatures_data) > 1 ? '' : 'style=display:none;' }}>Remove</button>
                    </div>
                </div>
            </div>
    ...
    <script>
        $(document).ready(function() {
           ...
           function calculateTotal($row) {
                ...
                $row.find('.total-input').val(total % 1 === 0 ? total.toFixed(0) : total.toFixed(2));
                calculateGrandTotal();
            }
            ...
            function calculateGrandTotal() {
                ...
                $('#grand_total').val(grandTotal % 1 === 0 ? grandTotal.toFixed(0) : grandTotal.toFixed(2));
            }
            ...
            $('#add_more_section').click(function() {
                ...
                $newSection.find('.temperature-select, .product-temperature-select')
